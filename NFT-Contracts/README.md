...
Three most important properties of Dreamoji which differentiate from other token standards
...
[View the Latest Spec](dreamoji_spec.md)
======================================
...
* `nftokensale` now batch as well
  - can list multiple dreamojis for sale as a batch
...
* `nftokensale`: callable ownly by owner; creates a sale listing in the token contract valid
...
* `purchase`: called when a user sends EOS to the dgood contract with a memo of "dgood_id,to_account"
...
* added name of the standard (dreamoji), version, and changed symbol type to symbol_code

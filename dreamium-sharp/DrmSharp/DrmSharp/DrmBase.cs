...
{
    /// <summary>
    /// Client wrapper to interact with drm blockchains.
    /// </summary>
    public class DrmBase
    {
        private DrmConfigurator DrmConfig { get; set; }
        private DrmApi Api { get; set; }
        ...
        public DrmBase(DrmConfigurator config, IHttpHandler httpHandler)
        {
            DrmConfig = config;
            if (DrmConfig == null)
            {
            ...
            Api = new DrmApi(DrmConfig, httpHandler);
        ...
        public async Task<SignedTransaction> SignTransaction(Transaction trx, List<string> requiredKeys = null)
        {
               ...
               trx.ref_block_prefix == 0)
            {
                ...
                var taposBlockNum = getInfoResult.head_block_num - (int)DrmConfig.blocksBehind;
                ...
                if ((taposBlockNum - getInfoResult.last_irreversible_block_num) < 2)
                {
                    ...
                    trx.expiration = getBlockResult.timestamp.AddSeconds(DrmConfig.ExpireSeconds);
                ...
                else
                {
                    ...
                    trx.expiration = getBlockHeaderState.header.timestamp.AddSeconds(DrmConfig.ExpireSeconds);
            ...
            {
                var availableKeys = await DrmConfig.SignProvider.GetAvailableKeys();
            ...
            var signatures = await DrmConfig.SignProvider.Sign(chainId, requiredKeys, packedTrx, abis);

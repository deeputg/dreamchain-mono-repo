...
import { AccountService, DialogsService, LoginService } from '../services'
...
export class FindAccountComponent implements OnInit {
  ...
  public accounts: AccountsByKeyModel
  public acoountsInf: AccountInfo[] = []
  public tokenArray: Array<{ token: string, balance: string, international: string }> = []
  public searchComplite: boolean
  public searchAccount: boolean
  public successName: boolean
  public successKey: boolean
  public errorName: boolean
  public errorKey: boolean
  public tokenInfo: string
  public errorMessage: string
  public procent: string
  public procentNum: number[]
  public netData: string[]
  public cpuData: string[]
  public tokenCut: string[]
  public staked: number
  public unstaked: number
  public result: AccountInfo
  public accountData: string
  public tokenStringTemp: string
  public tokenStringTemps: string[]
  ...
  public findAccountName (accountName: string) {
    this.procent = '0'
      ...
      data => {
        ...
        if (!isNull(this.result.refund_request)) {
          ...
          if (Date.parse(this.result.refund_request.request_time)) {
            const newDate = new Date(this.result.refund_request.request_time)
  ...
  public findAccountsKey (publicKey: string) {
    let iter = 0
    const tokenList = new Currency()
    this.successName = false
    this.successKey = false
    ...
    this.data.findByKey(body).subscribe(
      data => {
        ...
        this.accounts = data
        for (const index in this.accounts.account_names) {
  ...
  private setTokensGreymassSymbol (tokens) {
    const tokensArray = []
    tokens.forEach(rez => {
      const precision = rez.amount.toString().split('.')[1] ? rez.amount.toString().split('.')[1].length : 0
    ...
    if (tokens && tokens.length) {
      ...
      tokens.forEach(resultArr => {
        resultArr.forEach(element => {
          ...
          tokensArray.push({ international: name,  balance: amount })
        })
      })
      return tokensArray
    }
